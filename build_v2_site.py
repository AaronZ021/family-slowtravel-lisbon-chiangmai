...
def navlinks(active=""):
    items = [
        ...
        ("v2_side_by_side.html", "v2 横向对照"),
        ("v2_evidence_lisbon.html", "v2 里斯本证据"),
        ("v2_evidence_chiang_mai.html", "v2 清迈证据"),
        ("index.html", "← 返回 v1 站点"),
    ]
    out = []
...
<h2>v1 历史交付（v2 站点内副本）</h2>
<div class="card-grid">
  <div class="card">
    <a href="v2_side_by_side.html">
      <span class="tag">v1 事实对照</span>
      <h3>18 维度横向对照表</h3>
      <p>v1 纯事实左右两栏对照（v2 站点内副本）。</p>
    </a>
  </div>
  <div class="card">
    <a href="v2_evidence_lisbon.html">
      <span class="tag" style="color: var(--lisbon)">里斯本</span>
      <h3>里斯本 v1 全部证据</h3>
      <p>7 个证据 markdown 文件汇总（v2 站点内副本）</p>
    </a>
  </div>
  <div class="card">
    <a href="v2_evidence_chiang_mai.html">
  ...
  <div class="card">
    <a href="index.html">
      <span class="tag">其他 agent 站点</span>
      <h3>← 返回另一 agent 的 v1 主页</h3>
      <p>另一并行 session 写的 v1 入口与对应 8 个 HTML（无 v2_ 前缀）保留独立</p>
    </a>
  </div>
</div>
...
# v2 home page (under v2_ prefix to avoid clobbering other agent's index.html)
(SITE / "v2_index.html").write_text(wrap_page("v2 首页", index_body, "v2_index.html"))

# Reports — all prefixed v2_ to isolate from other agent's site
report_pages = [
    ("reports/evidence_side_by_side.md", "v2_side_by_side.html", "v2 证据横向对照表"),
    ("reports/final_comparison_report.md", "v2_final_report.html", "v2 最终对比报告"),
    ("reports/red_flags.md", "v2_red_flags.html", "v2 红旗清单"),
    ("reports/unknowns_and_next_verifications.md", "v2_unknowns.html", "v2 未知与下一步核实"),
]
for md, out, title in report_pages:
    (SITE / out).write_text(md_to_html(ROOT / md, title, out))

# v1 18-dim comparison matrix CSV (kept under v2_legacy_matrix name to not clobber)
(SITE / "v2_legacy_matrix.html").write_text(
    csv_to_html(
        ROOT / "reports/comparison_matrix.csv",
        "v1 18-维度对比矩阵（保留作参考）",
        "v2_legacy_matrix.html",
        intro="v1 18 维度红黄绿灰 + 置信度 + 关键源 ID。v2 的 37-indicator 矩阵在 v2_matrix.html。",
    )
)

# v1 evidence pages combined per city
lisbon_files = [
...
(SITE / "v2_evidence_lisbon.html").write_text(
    combined_evidence_page("lisbon", lisbon_files, "v1 里斯本全部证据 (v2 站点保留)", "v2_evidence_lisbon.html")
)
(SITE / "v2_evidence_chiang_mai.html").write_text(
    combined_evidence_page("chiang_mai", lisbon_files, "v1 清迈全部证据 (v2 站点保留)", "v2_evidence_chiang_mai.html")
)
...
# NOTE: 404.html, index.html, .nojekyll owned by other agent's build_site.py.
# This script writes only v2_*.html files to avoid clobbering them.
